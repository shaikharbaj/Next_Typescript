...
//get product........
export const loadAllUserAsync = createAsyncThunk("products/loadproducts", async (payload: Payload, thunkAPI) => {
    try {
        const { currentpage, searchTerm } = payload;
        const response = await privateRequest.get(`http://localhost:8000/user?page=${currentpage}&searchTerm=${searchTerm}`);
