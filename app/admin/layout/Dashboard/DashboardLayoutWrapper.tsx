...
> = ({ children }) => {
  ...
  return (
    <>
      <GuestWrapper>
        <div
          className={`relative min-h-screen pt-16 transition-all duration-500 ease-in-out ${
            toggleSidebar ? "md:pl-0" : "md:pl-72"
          }`}
        >
          <Header />
          <Sidebar
          ...
          />
          <div style={{ minHeight: "calc(100vh - 132px)" }}>{children}</div>
          <Footer />
        </div>
      </GuestWrapper>
    </>
